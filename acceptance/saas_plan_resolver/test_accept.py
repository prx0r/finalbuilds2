"""STANDARD alignment contract for saas_plan_resolver."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "saas_plan_resolver"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/saas_plan_resolver/align.py"

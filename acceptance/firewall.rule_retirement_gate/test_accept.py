"""STANDARD alignment contract for firewall.rule_retirement_gate."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "firewall.rule_retirement_gate"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/firewall.rule_retirement_gate/align.py"

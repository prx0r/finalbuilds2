"""STANDARD alignment contract for logs.query_alarm_proof."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "logs.query_alarm_proof"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/logs.query_alarm_proof/align.py"

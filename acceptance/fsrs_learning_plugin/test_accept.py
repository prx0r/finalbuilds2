"""STANDARD alignment contract for fsrs_learning_plugin."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "fsrs_learning_plugin"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/fsrs_learning_plugin/align.py"

"""STANDARD alignment contract for phone_verify_cli."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "phone_verify_cli"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/phone_verify_cli/align.py"

"""STANDARD alignment contract for dynamodb.vector_recall_gate."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "dynamodb.vector_recall_gate"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/dynamodb.vector_recall_gate/align.py"

"""STANDARD alignment contract for logs.geoip_drift_guard."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "logs.geoip_drift_guard"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/logs.geoip_drift_guard/align.py"

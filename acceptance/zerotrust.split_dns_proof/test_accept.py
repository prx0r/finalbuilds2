"""STANDARD alignment contract for zerotrust.split_dns_proof."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "zerotrust.split_dns_proof"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/zerotrust.split_dns_proof/align.py"

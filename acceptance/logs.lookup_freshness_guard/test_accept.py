"""STANDARD alignment contract for logs.lookup_freshness_guard."""
import importlib.util, pathlib
ROOT = pathlib.Path(__file__).resolve().parents[1]
PROD = ROOT / "platform" / "products" / "logs.lookup_freshness_guard"
def _load():
    p = PROD / "align.py"
    assert p.exists(), f"missing platform/products/logs.lookup_freshness_guard/align.py"
